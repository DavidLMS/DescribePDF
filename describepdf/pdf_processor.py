...
try:
    ...
    PIL_AVAILABLE = False
    logging.error("Pillow not installed. Install with 'pip install pillow'")
...
def get_pdf_pages(pdf_path: str) -> Tuple[Optional[pymupdf.Document], Optional[List[pymupdf.Page]], int]:
    ...
    if not PYMUPDF_AVAILABLE:
        logging.error("PyMuPDF is required for PDF processing but is not installed.")
        return None, None, 0
...
def render_page_to_image_bytes(page: pymupdf.Page, image_format: str = "jpeg", dpi: int = 150) -> Tuple[Optional[bytes], Optional[str]]:
    ...
    if not PYMUPDF_AVAILABLE or not PIL_AVAILABLE:
        logging.error("PyMuPDF and Pillow are required for image rendering but are not installed.")
        return None, None
    ...
    try:
        ...
        img_bytes_io.seek(0)
        logging.debug(f"Rendered page {page.number + 1} to {image_format.upper()} bytes.")
        return img_bytes_io.getvalue(), mime_type

    except Exception as e:
        logging.error(f"Error rendering page {page.number + 1} to image: {e}")
        return None, None
...
def extract_all_text(pdf_path: str) -> Optional[str]:
    ...
    """
    if not PYMUPDF_AVAILABLE:
        logging.error("PyMuPDF is required for text extraction but is not installed.")
        return None
...
def save_page_as_temp_pdf(original_doc: pymupdf.Document, page_num: int) -> Optional[str]:
    ...
    try:
        ...
        # We'll handle deletion manually in our cleanup process
        ...
        if temp_pdf_path and os.path.exists(temp_pdf_path):
            try:
                ...
                logging.debug(f"Cleaned up temporary PDF due to error: {temp_pdf_path}")
            except OSError:
                pass
        return None
